...
events.forEach(([f]) => inputs.push("-i", f.includes(":") || f.startsWith("/") ? f : W + f));
...
const cycleFilter = `${chains.join(";")};${mixIn}amix=inputs=${events.length}:normalize=0:duration=longest,apad=whole_dur=${CYCLE.toFixed(3)},atrim=0:${CYCLE.toFixed(3)}` + (music && !cut ? "[a]" : `,afade=t=out:st=${(CYCLE - (music ? 1.5 : 0.25)).toFixed(3)}:d=${music ? 1.5 : 0.25},alimiter=limit=0.95[a]`);
const target = music && !cut ? W + "cycle_fx.mp4" : OUT;
